...
export interface AuthContextValue {
  status: AuthStatus;
  me: MeResponse | null;
  /** True once the admin session has been fully resolved. */
  isAdmin: boolean;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const resolveMe = useCallback(async () => {
    ...
    if (response.user.role !== ROLES.ADMIN) {
      await clearSession();
      throw new Error('This account is not authorized to use the admin application.');
    }
    setMe(response);
  ...
  const value = useMemo<AuthContextValue>(
    () => ({
      status,
      me,
      isAdmin: status === 'authenticated' && me?.user.role === ROLES.ADMIN,
      login,
      logout,
      hasPermission,
      refreshMe,
